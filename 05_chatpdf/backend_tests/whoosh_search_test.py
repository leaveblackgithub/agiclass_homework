...
def write_test_search_txt(file_query_dict,top_n=2):
    from txt_parser_test import write_test_txt,add_ws_search_suffix
...
def get_ws_index_dir(index_name):
    return get_dir(TEMP_ROOT,ws_search,index_name)

def get_test_ws_connector(is_chinese=False,is_resplit=False):    
    if not is_chinese:        
    ...
    from txt_parser_test import get_test_paragraphs_from_txt,get_test_paragraphs_from_txt_resplit
    global test_ws_connector
    if test_ws_connector_name not in globals():
        test_ws_connector=MyWhooshSearchConnector(get_ws_index_dir(index_name),to_keywords)
        test_ws_connector.add_paragraphs(get_test_paragraphs_from_txt())
    return test_ws_connector


def get_test_ws_connector_resplit(is_chinese=False):    
    ...
    if test_ws_connector_resplit_name not in globals():
        test_ws_connector_resplit=MyWhooshSearchConnector(get_ws_index_dir,test_ws_connector_resplit_name)
